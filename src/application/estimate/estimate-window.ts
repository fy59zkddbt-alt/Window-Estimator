...
import { createWindow, type SingleWindowInput } from '../../domain/measurements/window/create-window';
import { priceGlazing } from '../../domain/pricing/glazing-pricing';

export function estimateWindow(input: SingleWindowInput, configuration: UserConfiguration): Calculation {
  ...
  if (measurement.hardwareId !== null) {
    const hardware = configuration.hardware.filter((item) => item.id === measurement.hardwareId);
  ...
  return { id: input.id, schemaVersion: 1, measurement, geometry, price,
